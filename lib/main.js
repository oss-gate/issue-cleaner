...
const main = async () => {
    ...
    const DOORKEEPER_GROUP = (0, core_1.getInput)("DOORKEEPER_GROUP");
    const doorkeeperEvents = DOORKEEPER_GROUP
        ? await (0, doorkeeper_1.getEvents)(DOORKEEPER_GROUP)
        : [];
    const CONNPASS_KEYWORD = (0, core_1.getInput)("CONNPASS_KEYWORD");
    const connpassEvents = CONNPASS_KEYWORD
        ? await (0, connpass_1.getEvents)({
    ...
    await Promise.all(issues.data.items.map(async (result) => {
        ...
        if (!((0, title_1.isWorkshop)(title) || (0, title_1.isMeetup)(title)))
            return;
        if ((0, date_1.isBefore)(new Date(), (0, title_1.getDate)(title)))
            return;
        const issue = {
            owner,
            repo,
